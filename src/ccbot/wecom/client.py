...
class WeComClient:
    ...
    def __init__(self, corp_id: str, secret: str, agent_id: int = 0) -> None:
    ...
    async def send_text_to_user(self, userid: str, content: str) -> dict:
        """Send a text message to an individual user.
    ...
    async def send_image_to_user(self, userid: str, media_id: str) -> dict:
        """Send an image message to an individual user."""
    ...
    async def send_markdown_to_user(self, userid: str, content: str) -> dict:
        """Send a markdown message to an individual user."""
        parts = split_text(content)
        result = {}
        for i, part in enumerate(parts):
            if len(parts) > 1:
    ...
    async def send_template_card_to_user(
        self,
        userid: str,
        *,
        title: str,
        ...
        """Send a button interaction template card to an individual user."""
        card: dict = {
    ...
    async def send_file(self, chatid: str, media_id: str) -> dict:
        """Send a file message to an application group chat."""
        return await self._post(
            "appchat/send",
            {
                "chatid": chatid,
    ...
    async def send_file_to_user(self, userid: str, media_id: str) -> dict:
        """Send a file message to an individual user."""
        return await self._post(
            "message/send",
            {
                "touser": userid,
